...
class Xdict(dict):
    def getKeys(self, value):
        ...
        valueList = list(self.values())
        resultIndex = [i for i in range(len(valueList)) if valueList[i] == value]
        return [keyList[i] for i in resultIndex]
        # 代码结束
...
class Vector:
    ...
    def __str__(self):
        # 请在此编写你的代码(可删除pass语句)
        return '(' + str(round(self.x,1)) + ',' + str(round(self.y,1)) + ',' + str(round(self.z,1)) + ')'
        # 代码结束
